...
4. Для статусов при необходимости закрывает активные незаблокированные override:
   ...
   - если сама override остаётся открытой с `effective_to = '9999-12-31 23:59:59'`
   - и только если последним событием стало `INSERT` в `model_status_source`
   - и вставленная source-запись после replacement-логики не вытеснена override по `source_record_id`.
...
- worker может закрыть только `model_status_override`;
- это допустимо только если одновременно выполнены все условия:
  - `is_final_override = false`;
  - `effective_to = '9999-12-31 23:59:59'`;
  - последним событием очереди был `INSERT` в `model_status_source`;
  - вставленная source-запись после replacement-логики не вытеснена override по `source_record_id`;
- при выполнении этих условий закрытие выполняется установкой `effective_to`, равного `effective_from` вставленной source-записи;
- сам `INSERT` в `model_status_override` никогда не должен менять `effective_to` этой же override-записи.
...
- если `source_record_id` заполнен, в таблице допускается только одна override-запись на одну source-запись;
- это правило зафиксировано частичным `UNIQUE INDEX` по `source_record_id`, применяемым только для `source_record_id is not null`;
- если нужно изменить уже существующую корректировку для конкретной source-записи, нужно делать `UPDATE` этой же строки и обновлять `updated_at`, а не создавать новую строку;
- если `source_record_id = null`, корректировка считается самостоятельной и может существовать в нескольких экземплярах.
...
- `source_record_id`:
  ...
  - если override для этого `source_record_id` уже существует, нужно обновлять существующую строку, а не вставлять вторую;
...
- закрытие выполняется установкой `effective_to`, равного `effective_from`
  новой source-записи.
- это произойдёт только если вставленная source-запись не вытеснена override по `source_record_id`.

### Что не должно происходить

- `INSERT` в `model_status_override` сам по себе не должен менять `effective_to` этой override-записи;
- source-статус, уже вытесненный override по `source_record_id`, не должен закрывать standalone override;
- `model_stage_override` не должна автоматически редактироваться воркером.
