...
/**
 * Обёртка для Camunda external task, которая создаёт корневой span для каждой задачи.
 * Это обеспечивает наличие trace_id и span_id для всех исходящих вызовов из обработчика.
 */
const withTracing = (callback, spanName = 'camunda-task') => {
  return async (task, taskService) => {
    const name = spanName || task.topicName || task.activityId || 'camunda-task';
    await runWithRootSpan(name, async () => {
...
/**
 * Декоратор для внешних задач Camunda, который:
 * - оборачивает выполнение в span (через withTracing)
 * - после выполнения обновляет статус и этап модели в БД
 */
const camundaExternalTaskStatusDecorator = (callback, bpmn, db, changeStage = false) => {
  return withTracing(async (task, taskService) => {
    await callback(task, taskService);
