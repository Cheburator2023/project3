...
SELECT t1.*,
       ...
       MT.DEPARTMENT_VALUE,
       MT.MIPM_VALUE,
       RM.ROOT_ASSIGNMENT_ID
FROM MODELS t1
         INNER JOIN
     ASSIGNMENT_X_MODEL RM
     ON
                 RM.MODEL_ID = t1.MODEL_ID
             AND
                 RM.ROOT_ASSIGNMENT_ID = ANY (:assignmentsIds::numeric[])
             AND
                 RM.EFFECTIVE_TO = TO_TIMESTAMP('9999-12-3123:59:59', 'YYYY-MM-DDHH24:MI:SS')
         INNER JOIN
         (select distinct model_id, bpmn_key_id from BPMN_INSTANCES) t2
         ON
         ...
         INNER JOIN (select t2.model_id,
                     ...
                     from (
                              select t1.model_id,
                                     ...
                                     t1.effective_from,
                                     row_number()
                                     over (partition by t1.model_id order by t1.effective_from desc) as rnrn_
                              from (
                                       ...
                                       from (
                                                select bbbiii.model_id,
                                                       ...
                                                       row_number()
                                                       over (partition by bbbiii.model_id order by bbbiii.effective_from desc) as rn_
                                       ...
                                       select status_2_.model_id,
                                              status_2_.status_,
                                              status_2_.effective_from
                                       from (
                                                select ar_.model_id,
                                                       case
                                                           when ar_.artefact_id = 52 and ar_.artefact_string_value =
                                                                                         'Нет, доработки не актуальны'
                                                               then 'Модель не эффективна в бизнес-процессе'
                                                           when ar_.artefact_id = 174 and
                                                                (ar_.artefact_string_value is null or ar_.artefact_string_value = 'false')
                                                               then 'Модель разработана, не внедрена'
                                                           when ar_.artefact_id = 201 and (ar_.artefact_value_id in (314, 315))
                                                               then ar_.artefact_string_value
                                                           WHEN ar_.artefact_id = 367 AND (ar_.artefact_value_id IN (519))
                                                               THEN 'Разработана, не внедрена'
                                                           WHEN ar_.artefact_id = 373 AND (ar_.artefact_value_id IN (411))
                                                               THEN 'Разработана, не внедрена'
                                                           WHEN ar_.artefact_id = 323 AND (ar_.artefact_value_id IN (426))
                                                               THEN 'Архив'
                                                           WHEN ar_.artefact_id = 351 AND (ar_.artefact_value_id IN (399))
                                                               THEN 'Архив'
                                                           WHEN ar_.artefact_id = 152 AND (ar_.artefact_value_id IN (35))
                                                               THEN 'Архив'
                                                           WHEN ar_.artefact_id = 323 AND (ar_.artefact_value_id IN (427))
                                                               THEN 'Разработана, не внедрена'
                                                           else NULL
                                                           end                                                           as status_,
                                                       ar_.effective_from,
                                                       row_number()
                                                       over (partition by ar_.model_id order by ar_.effective_from desc) as r_num
                                                from artefact_realizations ar_
                                                where ar_.artefact_id in (52, 201, 174, 367, 373, 323, 351, 152)
                                                  and ar_.effective_to =
                                                      TO_TIMESTAMP('9999-12-3123:59:59', 'YYYY-MM-DDHH24:MI:SS')
                                                  and case
                                                          when ar_.artefact_id = 52 and
                                                               ar_.artefact_string_value = 'Нет, доработки не актуальны'
                                                              then 'Модель не эффективна в бизнес-процессе'
                                                          when ar_.artefact_id = 174 and
                                                               (ar_.artefact_string_value is null or ar_.artefact_string_value = 'false')
                                                              then 'Модель разработана, не внедрена'
                     ...
                     where t2.rnrn_ = 1) ST
                    ON
                        t1.MODEL_ID = ST.MODEL_ID

         INNER JOIN (SELECT m.MODEL_ID,
                            m.ROOT_MODEL_ID,
                            m.MODEL_NAME,
                            m.MODEL_DESC,
                            m.MODEL_VERSION,
                            ...
                            m.UPDATE_AUTHOR,
                            m.PARENT_MODEL_ID,
                            MAX(CASE WHEN ar.ARTEFACT_ID = 6 THEN av.ARTEFACT_VALUE ELSE NULL END)         AS business_customer_departament,
                            MAX(CASE WHEN ar.ARTEFACT_ID = 7 THEN av.ARTEFACT_VALUE ELSE NULL END)         AS DEPARTMENT_VALUE,
                            MAX(CASE WHEN ar.ARTEFACT_ID = 83 THEN ar.ARTEFACT_STRING_VALUE ELSE NULL END) AS MIPM_VALUE
                     FROM MODELS m
                     ...
                     WHERE (
                         (
                                 ...
                                 and
                                     m.model_id in
                                     (
                                         select distinct m_tmp.MODEL_ID
                                         from MODELS m_tmp
                                                  INNER JOIN
                     ...
                     GROUP BY m.MODEL_ID,
                              ...
                              m.UPDATE_AUTHOR,
                              m.PARENT_MODEL_ID
                     ORDER BY m.MODEL_ID) MT
                    ON
                        t1.MODEL_ID = MT.MODEL_ID
WHERE t1.MODELS_IS_ACTIVE_FLG = :active
