...
WITH DepartmentsWithBpmn as (
    select coalesce(m.model_stage, bp_.BPMN_KEY_DESC) as BPMN_KEY_DESC,
        av_.ARTEFACT_VALUE as DEPARTMENT_NAME
    from BPMN_INSTANCES bi_
    inner join models m on bi_.model_id = m.model_id
...
SELECT DEPARTMENT_NAME,
       ...
       SUM(CASE WHEN BPMN_KEY_DESC IN ('initialization', 'Инициализация')
                                                               THEN 1 ELSE 0 END)::int as initialization,
       SUM(CASE WHEN BPMN_KEY_DESC = 'data' THEN 1 ELSE 0 END)::int                    as data,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('data_search', 'Поиск данных') 
                                                               THEN 1 ELSE 0 END)::int as data_search,
       SUM(CASE WHEN BPMN_KEY_DESC = 'data_pilot' THEN 1 ELSE 0 END)::int              as data_pilot,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('data_build', 'Построение витрины для разработки модели')
                                                               THEN 1 ELSE 0 END)::int as data_build,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('model', 'Разработка модели', 'Пилотирование модели') 
                                                               THEN 1 ELSE 0 END)::int as model,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('model_validation', 'Первичная валидация')
                                                               THEN 1 ELSE 0 END)::int as model_validation,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('integration', 'Разработка витрины для полной валидации', 'Продуктивизация модели', 'Тестирование на препрод и перенос на прод контур') 
                                                               THEN 1 ELSE 0 END)::int as integration,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('integration_datamart', 'Разработка промышленной витрины')
                                                               THEN 1 ELSE 0 END)::int as integration_datamart,
       ...
       SUM(CASE WHEN BPMN_KEY_DESC IN ('validation', 'Полная валидация')
                                                               THEN 1 ELSE 0 END)::int as validation,
       SUM(CASE WHEN BPMN_KEY_DESC IN ('removal', 'Вывод из эксплуатации') 
                                        THEN 1 ELSE 0 END)::int                        as removal,
       ...
       SUM(CASE WHEN BPMN_KEY_DESC IN ('cancel', 'Отмена разработки') 
                                                               THEN 1 ELSE 0 END)::int as cancel
