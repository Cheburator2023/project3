...
const formatModelsResult = ({ rows }) =>
  rows.map((row) => ({
    ...
    MODEL_STATUS_IMPLEMENTATION: row.NEW_MODEL_STATUS || getLastImplementationStatus(
      row.MODEL_STATUS_IMPLEMENTATION
    ),
    MODEL_STATUS: (row.NEW_MODEL_STAGE || row.NEW_MODEL_STATUS) ? row.NEW_MODEL_STAGE : getModelStatus(
      row.MODEL_STATUS,
      row.MODEL_STATUS_IMPLEMENTATION
    ),
