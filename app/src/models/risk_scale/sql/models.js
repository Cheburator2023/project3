...
SELECT
    ...
    FROM
        MODELS t1
    INNER JOIN RISK_SCALE_X_MODEL RM
    ON RM.MODEL_ID = t1.MODEL_ID
    AND RM.ROOT_RISK_SCALE_ID = :ROOT_RISK_SCALE_ID
    INNER JOIN
...
(
  SELECT unnest(string_to_array(:groups, ','))
)
  )
  )
...
ORDER BY
    ...
    WHERE
        ...
        AND
        t3.BPMN_KEY_DESC in (
            SELECT unnest(string_to_array(:type, ','))
        )
    ORDER BY
        t1.CREATE_DATE
`
